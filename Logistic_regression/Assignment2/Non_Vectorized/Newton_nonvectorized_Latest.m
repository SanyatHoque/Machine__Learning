...
for i = 1:max_iterations
 gradient =[0 0 0];   
    ...
    Cost_fun=0;
        ...
        for u = 1:3
            for w = 1:3
                newton(u) = newton(u) + (Hessian1(w,u)*gradient(w));
            end
        end
        %%
    ...
    prevtheta =theta;
    %%
    %% Calcute Cost Function
    %%Calculate Hypothesis5
%     Cost_fun = 0;
%     for mm = 1:80
%         zz=0;
%         for kk = 1:3
%             zz = zz +  (prevtheta(kk)*x(mm,kk));
%         end
%         h(mm) =  (1 / (1 + exp(-zz)));
%         Cost_fun = Cost_fun + (-y(mm)*log(h(mm))) - (1-y(mm))*log(1-h(mm)); %Cost Function
%     end
%     Cost_fun1 = (1/80)*Cost_fun  %Cost Function
    %%    
end
theta=prevtheta;
%%
