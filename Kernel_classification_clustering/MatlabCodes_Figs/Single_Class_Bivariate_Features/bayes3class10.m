%Maximum Likelihood Estimate : Naive Bayes Classifier for Bivariate Case,%Single Class
%Arbitrary Variance, non-diagonal elements equal to zero
...
%% Compute the Probability densitiy for each class and plot the contours
...
%%
%% 1D Gaussian Normal Distribution for X1
X11 = linspace(min(X1(1,1:end)), max(X1(1,1:end)));
...
%% 1D Gaussian Normal Distribution for X2
X22 = linspace(min(X2(1:end,1)), max(X2(1:end,1)));
...
% if Featurres X1, X2 are independant, Covariance with non zero
% non-diagonal elements are used.
% if Featurres X1, X2 are dependant, Variance with null components of
% non-diagonal elements are used.
for c = 1:3
    maximum_aposterior(c) = density_norm(x, class_mean(c,:), feature_covar(:,:,c))
end
...
%     maximum_aposterior1(c,feature) = density_norm(x(feature), class_mean(c,feature), feature_covar(feature,feature,c));
%     end
...
%Total Data X = 90 ; 
%Labelled Data for Class 1 = 30;
%Labelled Data for Class 2 = 30;
%Labelled Data for Class 3 = 30;
%Belief Probability for each Class (Uniform Priors) = 30/90 = 1/3 ;
maximum_aposterior = (1/3)*maximum_aposterior ; 
max(maximum_aposterior) ;                    %maximum posterior probability
find(maximum_aposterior == max(maximum_aposterior)) ; %index of the maximum aposterior 