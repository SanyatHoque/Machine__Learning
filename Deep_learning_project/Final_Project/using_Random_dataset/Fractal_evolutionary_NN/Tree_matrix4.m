function [Final_ouput_matrix102,Neuron_Connections_tree_matrix,m] = Tree_matrix4(f);
% clear all;clc;
% f = 5;  
% W1 = randi([1 2],f+1,f);
New_W1 = zeros(f+1,f);
...
%%%%%%%%%%%%%%%%
Final_ouput_matrix101 = [New_W1 New_W1a New_W1b New_W1c New_W1d New_W1e New_W1f New_W1g New_W2];
...
n2 = randi([2 4],1,1) ; % Synapses 2-4 from Nodes
for k = 1:n2 
    ss(k) = true ;
%     ss %Test
end
...
n3 = randi([2 4],1,1);  % Synapses 2-4 from Nodes
for k = 1:n3 
    ss(k) = true ;
%     ss %Test
end
...
n4 = randi([2 4],1,1);  % Synapses 2-4 from Nodes
for k = 1:n4 
    ss(k) = true ;
%     ss %Test
end
...
n5 = randi([2 4],1,1);  % Synapses 2-4 from Nodes
for k = 1:n5 
    ss(k) = true ;
%     ss %Test
end
...
for nn=1:m106    %Middle Matrix, nn = f column feature
ss = New_W1b(:,nn) ;
% Synapses 2-4 from Nodes. For each column m106, it makes
% n6 number of branches
n6 = randi([2 4],1,1);  % Synapses 2-4 from Nodes
for k = 1:n6 
    ss(k) = true ;
%     ss %Test
end
...
n7 = randi([2 4],1,1);  % Synapses 2-4 from Nodes
for k = 1:n7 
    ss(k) = true ;
%     ss %Test
end
...
% m108 = sum(New_W1f(1,1:end))   ;
for nn=1:m108    %Middle Matrix, nn = f column feature
ss = New_W1(:,nn) ;
% Synapses 2-4 from Nodes. For each column m108, it makes
% n8 number of branches
n8 = randi([2 4],1,1);  % Synapses 2-4 from Nodes
for k = 1:n8 
    ss(k) = true ;
%     ss %Test
end
...
Final_ouput_matrix102 = [New_W1 New_W1a New_W1b New_W1c New_W1d New_W1e New_W1f New_W1g New_W2];
%% Neuron Connection
